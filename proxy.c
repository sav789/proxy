...
void error(char* msg) // gives error
{
    perror(msg);
    exit(0);
}
...
int main(int argc,char* argv[])
{
    ...
    if(bind(sockfd,(struct sockaddr*)&serv_addr,sizeof(serv_addr))<0)//bind our address and port to it
        error("Error on binding");
    
    
    listen(sockfd,50);//get ready to accept with clients waiting to connect
    ...
    pid=fork();
    if(pid==0)//forks processes and children to do work
    {
